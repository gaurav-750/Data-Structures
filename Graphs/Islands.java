package Graphs;
import java.util.Arrays;
import java.util.Scanner;

public class Islands {

    public static void dfs(int[][] adMat, int currentVertex, boolean[] visitedArr){
        visitedArr[currentVertex] = true;// as we have arrived at this vertex, mark it as true.

        for (int i = 0; i < adMat.length; i++) {
            if (adMat[currentVertex][i] == 1 && !visitedArr[i]) {
                dfs(adMat, i, visitedArr);
            }
        }

        return;
    }

    public static int countConnectedGrpOfIslands(int[][] adMat){
        boolean[] visitedArr = new boolean[adMat.length];
        int countConnectedGrps = 0;

        for (int i = 0; i < adMat.length; i++) {
            if (!visitedArr[i]) {
                dfs(adMat, i, visitedArr);
//                System.out.println(Arrays.toString(visitedArr));
                countConnectedGrps++;
            }
        }

        return countConnectedGrps;
    }

    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);
        System.out.print("Enter the number of vertices: ");
        int n = sc.nextInt(); // n = number of vertices
        System.out.print("Enter the number of edges: ");
        int e = sc.nextInt(); // e = number of edges

        int[][] adMat = new int[n][n]; // creating a 2D-Matrix of n*n size

        // Taking inputs for all the edges:
        for (int i = 0; i < e; i++) {
            int v1 = sc.nextInt();
            int v2 = sc.nextInt();

            // There exist a connection between v1 and v2, that means there also exist a connection between
            // v2 and v1, hence we store 1 at both the cells in the matrix.
            adMat[v1][v2] = 1;
            adMat[v2][v1] = 1;
        }

        System.out.println(countConnectedGrpOfIslands(adMat));
    }
}
